...
class CurrentUserStore {
    ...
    onPageFault(pageFault) {
        ...
        if (!(pageFault instanceof InvalidResponseCode) || [400, 403, 404].indexOf(pageFault.statusCode) === -1) {
            ...
            if (process.env.NODE_ENV === 'production') {
                // TODO: You can log the errors here, for example one might use sentry for that
                // Note: To get current stores snapshot one can use: alt.takeSnapshot(). One can then
                //        restore the state locally by calling alt.bootstrap({snapshot...}). This enables
                //        us to easily reproduce errors.
            }
        }
    }
}
